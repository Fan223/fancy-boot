...
基于 Jackson 3（`tools.jackson.*`）的全局配置：忽略 `null` 字段、忽略 `isGetter` 序列化方法。
...
> starter 已包含 `spring-boot-starter-jackson`。Spring Boot 4 默认使用 Jackson 3。
> 注解（`@JsonInclude` 等）仍位于 `com.fasterxml.jackson.annotation.*` 包，Jackson 3 兼容 2.x 注解，无需切换 import。
...
**Q：业务实体类上的注解应该用哪个包？**
A：用 `com.fasterxml.jackson.annotation.*`。Jackson 3 在可预见未来都会保持这一注解包的兼容；改到
`tools.jackson.annotation.*` 等真正切换 Jackson 3 纯注解包时再做。

**Q：怎么关闭某个默认值？**
A：提供自己的 `JsonMapper` Bean 完整覆盖。starter 不提供细粒度配置开关（保持轻量）。

**Q：怎么调试序列化输出？**
A：在 logback/log4j2 中调整 `tools.jackson.databind` 的 logger 级别到 DEBUG。
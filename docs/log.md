...
> starter 已默认使用 **Log4j2**（排除 spring-boot 默认 logback），无需业务项目额外配置日志框架。
...
| 功能             | 描述                                          | 默认开关 |
|----------------|---------------------------------------------|------|
| `@Log` 切面      | 在方法上加 `@Log` 主动开启日志                         | ✅    |
| Controller 切面  | 自动为所有 Controller 方法记录日志（除非显式 `@Log`）        | ✅    |
...
fancy:
  log:
    enabled: true                       # 全局总开关
    service-name: order-service         # 自定义服务名(可选,不配则取 spring.application.name)
...
如果某个 Controller 方法已有 `@Log`，**优先使用 `@Log` 的配置**，不会重复打印。
...
1. 从请求头 `X-Trace-Id` 读取 TraceId，没有则使用 Snowflake ID 生成
2. 写入 Log4j2 的 `ThreadContext`（MDC），日志格式中输出
3. 在响应头 `X-Trace-Id` 中回写给客户端

业务日志格式中需要包含 `%X{traceId}` 占位符，例如：
...
提供自定义的 `LogAdvice` / `LogAspect` Bean 即可替换默认实现（`@ConditionalOnMissingBean`）。
...
public LogAdvice logAdvice(LogPrinter printer) {
    return new LogAdvice(properties, printer) {
        @Override
        protected void before(LogEvent event) {
            // 自定义前置逻辑
        }
    };
}
...
**Q：为什么默认使用 Log4j2 而不是 Logback？**
A：Log4j2 异步日志性能更好，社区共识。starter 自动排除了 spring-boot 默认引入的 logback。
